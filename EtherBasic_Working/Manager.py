...
def main():
    ...
    print("The exact source code of the contract you are expected to have deployed can be found here: FILL")
    ...
    print("\nPlease enter the public key of the Ethereum acconut you'd like to use.")
    print("Note that this public key must be the 'owner' of the contract you deployed.\n")
    manager_ethereum_public_key = input()
    ...
    while True:
        etherscan_logs = requests.get("https://api-rinkeby.etherscan.io/api?module=logs&action=getLogs&fromBlock=" \
              + str(start_block) + "&toBlock=latest&address=0xbd05ed605587c9e25128955f3c262c694291a884&apikey=" \
                    ...
                    for tx in block_txs:
                        if "assetCode" in tx and "hub" in tx:
                            if tx["assetCode"] == "wei" and tx["hub"] == manager_topl_public_key \
                                ...
                                for prop in tx["to"]:
                                    if prop["proposition"] == \
                                            "22222222222222222222222222222222222222222222":  # burn box
                                        print("Wei burned!")
                                        print(tx)
                                        nonce = w3.eth\
